...
public class main {
	...
	public static void main(final String[] args) {
		...
		try {
			...
			LOGGER.log(Level.INFO, Constants.PROJECT_NAME + " started with pid:" + pid.split("@")[0]);
			String pidfile = cfg.getPidFile();
			try {
				Utility.writePid(pid.split("@")[0].toString(), pidfile);
			} catch (Exception e) {
				LOGGER.log(Level.CONFIG, "Error while trying to write pidfile " + e);
			}

			Locale.setDefault(Locale.US);
			DBConn[] myDBConn = cfg.getConnections();
			if (myDBConn == null || myDBConn.length == 0) {
				LOGGER.log(Level.CONFIG, "ERROR on main - Connections is empty or null");
			}
			...
			try {
				...
				LOGGER.log(Level.WARNING, "MaxThread not defined calculated maxThread = " + myDBConn.length * 3);
			}
			if (maxThread == null)
				maxThread = 0;
			if (maxThread == 0) {
			...
			while (true) {
				...
				for (int i = 0; i < myDBConn.length; i++) {
					...
					Hashtable<String, Integer> zabbixServers = c
							.getZabbixServers();
					LOGGER.log(Level.CONFIG, "Ready to run DBJob for dbname ->" + myDBConn[i].getName());
