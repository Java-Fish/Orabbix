...
*/
	...
	public void run() {
            try {
                ...
                if (!stopping) {
					SmartLogger.logThis(Level.WARNING, "ignoring exception"+ e);
                }
        ...
        while (queue.size() > 0) {
            ...
            if (retryCount==retryNumber){
            	SmartLogger.logThis(Level.WARNING, "Error i didn't sent item "
						+ item.getKey() + "  on host " + host + " tried "
						+ retryCount);
            	//Sotre on database for nex retry
            }
        }
    }
    ...
    private void send(final String key, final String value) throws IOException {
        ...
        if (SmartLogger.isLogLevelEnable(Level.CONFIG)) {
        	SmartLogger.logThis(Level.CONFIG,"sending " + message);
        }
        ...
        while (serverlist.hasMoreElements()){
	        ...
	        try {
	            ...
	            if (SmartLogger.isLogLevelEnable(Level.CONFIG)) {
	            	SmartLogger.logThis(Level.CONFIG, "received "
							+ new String(response));
	            }
