/*
 * This is a fork from Andrea Dalle Vacche
 * 
 * Copyright (C) 2022 Maximilian Fischer.
 * 
 * == Copyright from Andrea Dalle Vacche ==
 * 
...
public class DBEnquiry {
	...
	private static final Logger LOGGER = Logger.getLogger(DBEnquiry.class.getName());
	...
	public static String ask(String _query, Connection _con, String queryName,
		...
		try{
			...
			try{
				...
				LOGGER.log(Level.SEVERE, "Error on DBEnquiry while closing resultset "
	...
	public static ZabbixItem[] execute(Query[] _queries, Connection _conn,
			...
			try {
				...
				if (_queries[i].getActive()) {
					...
					if (now.after(nextRun)) {
						...
						LOGGER.log(Level.CONFIG, "Actual query is "
						...
						if (racecond) {
							...
							SZItems.add(zitem);
							LOGGER.log(Level.CONFIG, "I'm going to return " + result
							+ " for query "
							+ _queries[i].getName()
							+ " on database=" + dbname);
						}
				...
				LOGGER.log(Level.SEVERE, "Error on DBEnquiry on query=" + _queries[i].getName()
				+ " on database=" + dbname
				...
				ZabbixItem zitem = new ZabbixItem(_queries[i].getName(),
						tempStr , dbname);
				SZItems.add(zitem);
				LOGGER.log(Level.CONFIG, "I'm going to return "
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "Error on DBEnquiry while closing connection "
			+ ex.getMessage() + " on database=" + dbname);
		}
