			...
			try {
				...
				if (_queries[i].getActive()) {
					...
					if (now.after(nextRun)) {
						...
						SmartLogger.logThis(Level.CONFIG, "Actual query is "
						...
						if (_queries[i].getRaceQuery() != null) {
							if (_queries[i].getRaceQuery().length() > 0) {
								SmartLogger.logThis(Level.CONFIG, "INFO:"
										+ _queries[i].getName()
										+ " RaceCondiftionQuery ->"
						...
						if (racecond) {
							...
							ZabbixItem zitem = new ZabbixItem(Constants.ITEM_PREFIX+_queries[i]
				...
				ZabbixItem zitem = new ZabbixItem(Constants.ITEM_PREFIX+_queries[i].getName(),
						tempStr , dbname);
				SZItems.add(zitem);
				SmartLogger.logThis(Level.CONFIG, "I'm going to return "
