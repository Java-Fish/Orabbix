...
public class DBJob implements Runnable {
	...
	private static final Logger LOGGER = Logger.getLogger(DBJob.class.getName());
	...
	private boolean Alive(Connection _conn){
		try {
			...
			e.printStackTrace();
			LOGGER.log(Level.CONFIG, "Database "
	...
	public void run() {
		...
		LOGGER.log(Level.CONFIG, "Starting dbJob on database "
...
/*			if (dbConn.isClosed()){
			...
			if (Alive(dbConn)){
				...
				if (zitems != null && zitems.length > 0) {
					LOGGER.log(Level.CONFIG, "Item retrieved "
					...
					for (int cnt = 0; cnt < zitems.length; cnt++) {
						...
						LOGGER.log(Level.CONFIG, "dbname " + this._dbname
				...
				LOGGER.log(Level.SEVERE, "Error on dbJob for database "
				+ _dbname + " " + _queriesGroup + " error: " + e);
			} finally {
				if (_queries != null)
					_queries = null;
			}
			LOGGER.log(Level.INFO, "Done with dbJob on database "
