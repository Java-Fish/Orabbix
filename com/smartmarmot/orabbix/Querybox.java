...
public class Querybox {
	...
	private static final Logger LOGGER = Logger.getLogger(Querybox.class.getName());
	...
	public Querybox(String _localdbname, String _prp) {
		try {
			...
			if (_localdbname != null && _localdbname.length() > 0) {
				this.setDBName(_localdbname);
			}

		} catch (Exception e) {
			LOGGER.log(Level.SEVERE, "Error on QueryBox "
			+ e.getMessage());
		}
