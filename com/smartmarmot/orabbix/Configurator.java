...
public class Configurator {
	...
	private static final String LOG_MESSAGE_CONF_RETRIVING_LIST = "Error on Configurator while retriving the databases list ";
	...
	protected static Properties getPropsFromFile(String _filename) {
		try {
			...
			if (_filename != "" && _filename != null) {
				...
				queryFile = null;
				LOGGER.log(Level.CONFIG, "Loaded the properties from " + _filename);
				propsq.load(fisq);
				fisq.close();
				return propsq;
			}
		} catch (Exception e) {
			LOGGER.log(Level.SEVERE, "Error on Configurator reading properties file getPropsFromFile("
	...
	private static Query[] getQueries(String parameter, Properties _propsq)
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator on reading properties file "+ _propsq.toString() +" getQueries("
	...
	private static Query getQueryProperties(Properties _propsq,
		...
		try {
			...
			try {
				...
				LOGGER.log(Level.SEVERE, "Error while getting " + _queryName + "."
			...
			try {
				...
				LOGGER.log(Level.CONFIG, "Note: " + _queryName
				+ "." + Constants.QUERY_NO_DATA_FOUND
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator on getQueryProperties("
		...
		for (int i = 0; i < _myquery.length; i++) {
			try {
				...
				if (!_myquery[i].getSQL().equalsIgnoreCase(qnew.getSQL())) {
					...
					LOGGER.log(Level.INFO, "Statement changed on query "
				...
				if ((_myquery[i].getActive().equals(true) && qnew.getActive()
					...
					LOGGER.log(Level.INFO, "Status of query "
				...
				if (_myquery[i].getPeriod() != qnew.getPeriod()) {
					...
					LOGGER.log(Level.INFO, "Period of query "
					+ _myquery[i].getName() + " has changed ->"
					+ qnew.getPeriod());
				}
				if (!_myquery[i].getRaceQuery().equalsIgnoreCase(
					...
					LOGGER.log(Level.INFO, "RaceQuery for query "
					+ _myquery[i].getName() + " has changed ->"
					+ qnew.getRaceQuery());
				}
				if (!_myquery[i].getRaceValue().equalsIgnoreCase(
						qnew.getRaceValue())) {
					_myquery[i].setRaceQuery(qnew.getRaceQuery());
					LOGGER.log(Level.INFO, "RaceValue of query "
					+ _myquery[i].getName() + " has changed ->"
					+ qnew.getRaceValue());
				}
			} catch (Exception e) {
				LOGGER.log(Level.SEVERE, "Error on Configurator while getting getQueryProperties( "
	...
	public Configurator(String _url) throws IOException {
		...
		try {
			try {
				...
				LOGGER.log(Level.SEVERE, "Error on Configurator while retriving configuration file "
				+ _url + " " + e.getMessage());
			}
			_props = props;
		} catch (Exception e) {
			LOGGER.log(Level.SEVERE, "Error on Configurator " + e.getMessage());
		}
	}
	...
	private DBConn getConnection(String dbName) throws Exception {
		try {
			...
			LOGGER.log(Level.CONFIG, "getConnection for database " + dbName);
			String url = "";
			try {
				url = new String(_props.getProperty(dbName + "."
						+ Constants.CONN_URL));
			} catch (Exception ex) {
				LOGGER.log(Level.SEVERE, "Error on Configurator getConnection while getting "
			...
			try {
				...
				LOGGER.log(Level.CONFIG, "Note: " + dbName + "."
				...
				try {
					...
					LOGGER.log(Level.WARNING, "Warning I will use default value "
					+ maxActive);
				}
			}
			tds.setMaxTotal(maxActive.intValue());
			...
			tds.setDefaultMaxIdle(maxIdle.intValue());
			
			LOGGER.log(Level.INFO, "DB Pool created: " + tds);
			LOGGER.log(Level.INFO, "URL=" + url);
			LOGGER.log(Level.INFO, "maxPoolSize=" + tds.getDefaultMaxTotal());
			LOGGER.log(Level.INFO, "maxIdleSize=" + tds.getDefaultMaxIdle());
			LOGGER.log(Level.INFO, "maxIdleTime=" + tds.getDefaultMinEvictableIdleTimeMillis() + "ms");
			LOGGER.log(Level.INFO, "poolTimeout=" + tds.getDefaultMaxWait());
			LOGGER.log(Level.INFO, "timeBetweenEvictionRunsMillis=" + tds.getDefaultTimeBetweenEvictionRunsMillis());
			LOGGER.log(Level.INFO, "numTestsPerEvictionRun=" + tds.getDefaultNumTestsPerEvictionRun());
			...
			LOGGER.log(Level.INFO, "Connected as " + tempStr);
			...
			LOGGER.log(Level.INFO, "--------- on Database -> " + tempStr);
			con.close();
			DBConn mydbconn = new DBConn(tds, dbName.toString());
			return mydbconn;

		} catch (Exception ex) {
			LOGGER.log(Level.SEVERE, "Error on Configurator for database " + dbName
			+ " -->" + ex.getMessage());
			return null;
		}
	}
		...
		try {
			...
			for (int i = 0; i < DatabaseList.length; i++) {
				...
				if (dbc!=null){
					...
					LOGGER.log(Level.INFO, "This Database "+DatabaseList[i]+" removed");	
	        	}
			}
			// fis.close();
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator getConnections "
			+ ex.getMessage());
			return null;
		}
	}
	...
	public String[] getDBList() throws Exception {
		try {
			...
			try {
				...
				LOGGER.log(Level.SEVERE, LOG_MESSAGE_CONF_RETRIVING_LIST
	...
	public Integer getMaxThread() throws Exception {
		try {
			verifyConfig();
			return Integer.parseInt(_props.getProperty(Constants.ORABBIX_DAEMON_THREAD));
		} catch (Exception ex) {
			LOGGER.log(Level.SEVERE, "Error on Configurator while retriving the "
	...
	public String getPidFile() throws Exception {
		try {
			verifyConfig();
			String pidfile = _props.getProperty(Constants.ORABBIX_PIDFILE);
			LOGGER.log(Level.INFO, "PidFile -> " + pidfile);

			if (pidfile.equals("")) {
				LOGGER.log(Level.SEVERE, "Error retrieving pidfile from " + _props);
			}
			return pidfile;

		} catch (Exception ex) {
			LOGGER.log(Level.SEVERE, "Error on Configurator getPidFile " + ex);
			return null;
		}
	}

	public String getQueryFile() {
		return _props.getProperty(Constants.QUERY_LIST_FILE);
	}
	...
	public String getQueryFile(String dbName) {
		try {
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator on getQueryFile("
			+ dbName + ") " + ex.getMessage());
			LOGGER.log(Level.WARNING, "I'm going to return getQueryFile() ");
			return getQueryFile();
		}
		return null;
	}
	...
	public Properties getQueryProp() {
		...
		try {
			FileInputStream fisq;
			String fiqp = _props.getProperty(Constants.QUERY_LIST_FILE);
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator  while getting "
		...
		try {
			...
			if (dbname != null  && !dbname.equals("")) {
				...
				fisq = new FileInputStream(new java.io.File(queryFile
						.getAbsoluteFile().getCanonicalPath()));
				queryFile = null;
				LOGGER.log(Level.CONFIG, "Debug loaded the " + dbname + "."
				+ Constants.QUERY_LIST_FILE);
				...
				LOGGER.log(Level.CONFIG, "Debug I'm loading the default "
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator getting "+Constants.QUERY_LIST_FILE +" " + dbname
			+ e.getMessage());
			return null;
		}
	}
	...
	public String getZabbixServer() throws Exception {
		try {
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator while retriving zabbix server host "
	...
	public int getZabbixServerPort() throws Exception {
		try {
			verifyConfig();
			Integer port = Integer.parseInt(_props.getProperty(Constants.ZABBIX_SERVER_PORT));
			return port.intValue();
		} catch (Exception ex) {
			LOGGER.log(Level.SEVERE, "Error on Configurator while retriving zabbix server port "
			+ Constants.ZABBIX_SERVER_PORT + " " + ex);
			LOGGER.log(Level.WARNING, "I will use the default port "
	...
	public Hashtable<String, Integer> getZabbixServers() throws Exception {
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "Error on getZabbixServers while getting "
		...
		while (st.hasMoreTokens()) {
			...
			Integer port = Constants.ZABBIX_SERVER_DEFAULT_PORT;
			try {
				port = Integer.parseInt(_props.getProperty(token + "." + Constants.ZABBIX_SERVER_PORT));
			} catch (Exception e) {
				LOGGER.log(Level.WARNING, "Warning on getZabbixServers while getting " + token
				+ "." + Constants.ZABBIX_SERVER_PORT + " "
				+ e.getMessage());
				LOGGER.log(Level.WARNING, "Warning I will use the default port"
				+ port);
			}
	...
	public boolean hasQueryFile(String dbName) {
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator getting"+ Constants.QUERY_LIST_FILE
			+ dbName + ex.getMessage());
			return false;
		}
		return false;
	...
	public boolean isEqualsDBList(DBConn[] _dbc) throws Exception {
		try {
			...
			LOGGER.log(Level.SEVERE, "Error on Configurator while comparing the databases lists on isEqualsDBList "
			+ ex);
			return false;
		}
	}
		...
		try {
			...
			for (int j = 0; j < localdblist.length; j++) {
				...
				if (!ArrayUtils.contains(remotedblist, localdblist[j])) {
					 ...
					 * adding database
					 */
					LOGGER.log(Level.INFO, "New database founded! "
					+ localdblist[j]);
			...
			for (int x = 0; x < _dbc.length; x++) {
				if (!ArrayUtils.contains(localdblist, _dbc[x].getName())) {
					LOGGER.log(Level.WARNING, "Database " + _dbc[x].getName()
	...
	public static Integer  getSleep() throws Exception {
		try{
			...
			Integer sleep = 5;
			try{
				sleep=Integer.parseInt(_props.getProperty(Constants.ORABBIX_DAEMON_SLEEP));
			}catch (Exception e){
				LOGGER.log(Level.WARNING, "Warning while getting "+Constants.ORABBIX_DAEMON_SLEEP+" I will use the default "+sleep);
			}
			return sleep;

		} catch (Exception ex){
			LOGGER.log(Level.SEVERE, "Error on Configurator while retriving "+Constants.ORABBIX_DAEMON_SLEEP+" "+ex);
			return null;
		}
	}
