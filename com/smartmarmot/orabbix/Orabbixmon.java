		...
		try {
			...
			while (running) {
				...
				for (int i = 0; i < myDBConn.length; i++) {
					...
					DBConnManager _dbcm = myDBConn[i].getDBCM();
					...
					SmartLogger.logThis(
							...
							"Ready to run DBJob for dbname ->"
									+ myDBConn[i].getName());
					Runnable runner = new DBJob(_dbcm, q, Constants.QUERY_LIST,
							zabbixServers, myDBConn[i].getName());
					executor.execute(runner);

				}// for (int i = 0; i < myDBConn.length; i++) {
				Thread.sleep(60 * 1000);
				SmartLogger.logThis(Level.CONFIG, "Waking up Goood Morning");
			}
		} catch (Exception e1) {
