...
public final class Sender implements Runnable {
	private static final Logger LOGGER = Logger.getLogger("Orabbix");
...
*/
    ...
    public void run() {
            try {
                ...
                while (retryCount<= retryNumber){
	                try{
	                	...
	                	break;
							...
							LOGGER.log(Level.WARNING, "Warning while sending item " + item.getKey()
	                		...
	                		retryCount++;
	                		 if (retryCount==retryNumber){
								 LOGGER.log(Level.WARNING, "Error i didn't sent item " + item.getKey()
                ...
                if (!stopping) {
					LOGGER.log(Level.WARNING, "ignoring exception" + e);
                }
        ...
        while (queue.size() > 0) {
            ...
            if (retryCount==retryNumber){
				LOGGER.log(Level.WARNING, "Error i didn't sent item "
    ...
    private void send(final String key, final String value) throws IOException {
        ...
        if (LOGGER.isLoggable(Level.CONFIG)) {
			LOGGER.log(Level.WARNING, "sending " + message);
        }
        ...
        while (serverlist.hasMoreElements()){
	        ...
	        try {
	            ...
	            if (LOGGER.isLoggable(Level.CONFIG)) {
					LOGGER.log(Level.CONFIG, "received "
					+ new String(response));
	            }
	            if (read != 2 || response[0] != 'O' || response[1] != 'K') {
					LOGGER.log(Level.WARNING, "received unexpected response '"
					+ new String(response) + "' for key '"
					+ key + "'");
	            }
	        } catch (Exception ex){
				LOGGER.log(Level.SEVERE, "Error contacting Zabbix server " + zabbixServer
				+ "  on port "
